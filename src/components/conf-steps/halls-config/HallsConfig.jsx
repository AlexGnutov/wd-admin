...
function HallsConfig() {
  const {
    rows, seatsPerRow, seats, activeHall,
  } = useSelector((state) => state.hallConfig);
  ...
  return (
    <DashboardBlock header="Конфигурация залов">
      ...
      <div className="conf-step__legend">
        ...
        <label className="conf-step__label" htmlFor="seatsPerRow">
          Мест, шт
          <input
            name="seatsPerRow"
            type="number"
      ...
      <div className="conf-step__hall">
        <div className="conf-step__hall-wrapper">
          {seats.map((row, rowId) => (
            <div key={v4()} className="conf-step__row">
              {row.map((seat, colId) => (
                <Chair
